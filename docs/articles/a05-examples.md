...
To calculate adherence for each medication group the groups must be
defined first:
...
For selecting people from specific cohorts, use a cohort table reference
and cohort id values.
...
medicationGroups <- medicationGrouping(cdm, 
                             ingredientGroups=list(
                               simvastatin_rosuvastatin = c("Simvastatin", "Rosuvastatin"),
                               other_statins = c("Atorvastatin", "Pravastatin", "Fluvastatin", "Lovastatin", "Pitavastatin")
                               ))
```
...
Adding cohorts as features to CMA data requires cohort table as input
variable. It is recommended to use CDMConnector for generating this.
...
cma_values_with_cohort_features <- addCohorts(adherenceCMA5, cdm$cohort)
```
...
To get an overview this function reports the number of individuals in
three groups: (1) individuals included in the cohort,

2.  individuals with calculated adherence values,

3.  individuals who have at least *n* drug exposure records within a
    given calendar year, calculated from the generated drug exposure
    table.
