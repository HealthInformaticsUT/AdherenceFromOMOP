...
AdherenceFromOMOP requires several packages that may not be automatically installed. Install them with:

```{r, eval = FALSE}
# Core dependencies
install.packages(c("dplyr", "DBI", "cli", "rlang", "magrittr", "tidyr", "lubridate", "data.table"))

# Database connectors (install the one you need)
install.packages("RPostgres")  # For PostgreSQL
install.packages("duckdb")     # For DuckDB (useful for testing)

# OMOP-related packages
...
Medication groups can be defined to calculate adherence by therapeutic category. Here, we reuse the earlier drugIngredientCodes object. To compute adherence, for large datasets use the batched functions.
