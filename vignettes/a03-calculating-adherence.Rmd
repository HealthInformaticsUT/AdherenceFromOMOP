...
results <- adherenceCalculationsLoader(chronicDrugExposure, cdm, resultsTableName = "AdherenceFromOMOP_results_table_name", batchSize = 1000, cma = c("CMA9"))
results
```

## Medication adherence with sliding window

This approach computes adherence over a series of sliding windows within the observation period, allowing adherence to be examined over time.

```{r, eval = FALSE, message = FALSE}
results <- adherenceSlidingWindowLoader(chronicDrugExposure, cdm, resultsTableName = "AdherenceFromOMOP_results_table_name", batchSize = 1000, cma = c("CMA5"))
results
```
...
medicationGroup <- list(
  c(1, 2),
  c(2, 3, 4)   # vectors of drug_concept_ids
)
